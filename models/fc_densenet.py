...
# Copied from https://gist.github.com/felixgwu/045c887b6ccdf0edf4648da0c40bcc12
...
if __name__ == '__main__':
    ...
    dummy = np.random.rand(1, 3, 350, 525).astype('float32')
    dummy_tensor = torch.from_numpy(dummy).to(device)
    print(dummy_tensor.shape)
    mask, classes = model(dummy_tensor)
    print(f'Mask shape: {mask.shape}')
    print(f'Classes shape: {classes.shape}')
    del model, dummy, dummy_tensor
    print(mask)
    print()
    print(classes)
    wait = input('')